...
const watchVideosInSequence = async (page, ipAddr, targetUrlsList, durationInSeconds, port) => {
  for (const url of targetUrlsList) {
    ...
    logger.info(`[port ${port}] Navigating to ${url} via Tor IP ${ipAddr} (target: ${durationInSeconds}s)`);
    ...
    try {
      ...
      logger.info(`[port ${port}] Watching ${url} for ${duration.toFixed(0)}s (traffic via socks5 proxy)...`);
